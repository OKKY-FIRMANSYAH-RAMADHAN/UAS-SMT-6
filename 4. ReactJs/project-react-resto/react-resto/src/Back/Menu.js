...
const Menu = () => {
    ...
    async function showData(id) {
        ...
        setGambar(<img src={res.data[0].gambar} alt="" width="250" />);
    ...
    return (
        <div>
            <div className="row">
                <h2 className="text-uppercase text-center">Data Menu</h2>
            </div>
            <div className="row">
                <div>
                    <p>{pesan}</p>
                    ...
                    <form onSubmit={handleSubmit(simpan)}>
                        <div className="mb-3">
                            ...
                            <select name="idkategori" ref={register} className="form-control">
                                {
                                    kategori.map((val, index) => val.idkategori === idkategori ? <option selected key={index} value={val.idkategori}>{val.kategori}</option> : <option key={index} value={val.idkategori}>{val.kategori}</option>)
                                }
                        ...
                        <div className="mb-3">
                            <input type="submit" className="btn btn-primary form-control" name="submit" />
                        </div>
                    </form>
                </div>
                <div className="col-4">
                    ...
                    <tbody>
                        {
                            isi.map((val, index) => (
                                <tr key={index}>
                                    ...
                                    <td>{val.kategori}</td>
                                    <td>{val.menu}</td>
                                    <td><img width="100" src={val.gambar} alt="" /></td>
                                    <td>{val.harga}</td>
                                    <td><button onClick={() => hapus(val.idmenu)} className="btn btn-danger">Hapus</button></td>
